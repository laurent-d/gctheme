  ...
  SearchableOptionList.prototype = {
    ...
    defaults: {
      ...
      events: {
        ...
        onChange: undefined,
        onScroll: function () {
          var $modalContentParent = this.$input.parents('.modal-content');
          var $modalTopPosition = 0,
              $modalLeftPosition = 0;
          if ( $modalContentParent.length > 0 ){
            this.config.scrollTarget = $('.modal-body');
            var $modalTopPosition = $modalContentParent.offset().top,
                $modalLeftPosition = $modalContentParent.offset().left;
          }
          var selectionContainerYPos = this.$input.offset().top - $modalTopPosition - this.config.scrollTarget.scrollTop() + this.$input.outerHeight(false),
              selectionContainerXPos = this.$container.offset().left - $modalLeftPosition,
              selectionContainerHeight = this.$selectionContainer.outerHeight(false),
              displayContainerAboveInput = (document.documentElement.clientHeight / 2) < ($(this.$input).offset().top - $(window).scrollTop()),
              ...
              /* Display above if select is in half bottom of the page
              *   document.documentElement.clientHeight / 2 : half the viewport height
              *   $(this.$input).offset().top: height betwen the input and the top of the document
              *   $(window).scrollTop(): height between the viewport's top and the top of the document
              */
              displayContainerAboveInput = (document.documentElement.clientHeight / 2) < ($(this.$input).offset().top - $(window).scrollTop()),
              // Change for Eventmaker
              // selectionContainerWidth = this.$innerContainer.outerWidth(false) - parseInt(this.$selectionContainer.css('border-left-width'), 10) - parseInt(this.$selectionContainer.css('border-right-width'), 10);
              selectionContainerWidth = this.$innerContainer.outerWidth(false);
              if (displayContainerAboveInput) {
                // position the popup above the input
                selectionContainerYPos = this.$input.offset().top - $modalTopPosition - selectionContainerHeight - this.config.scrollTarget.scrollTop() + parseInt(this.$selectionContainer.css('border-bottom-width'), 10);
                this.$container
              ...
              this.$selectionContainer
                  ...
                  .css('left', Math.floor(selectionContainerXPos))
    ...
    _registerWindowEventsIfNeccessary: function () {
      if (!window[this.WINDOW_EVENTS_KEY]) {
        $(document).click(function (event) {
          ...
          $('.sol-active').not($clickedWithinThisSolContainer)
                          .each(function (index, item) {
                            $(item).data(SearchableOptionList.prototype.DATA_KEY)
                                    .close();
                          });
        });
    ...
    _initializeUiElements: function () {
      ...
      this.$selectionContainer = $('<div class="sol-selection-container"/>').append(this.$noResultsItem)
                                                                            .append(this.$loadingData)
                                                                            .append(this.$selection);
      this.$container = $('<div class="sol-container"/>').hide().data(this.DATA_KEY, this)
    ...
    _addSelectionDisplayItem: function ($changedItem) {
      ...
      if (!$existingDisplayItem) {
        ...
        if (solOptionItem.color) {
          $existingDisplayItem.css('color', '#FFF')
              .css('background-color', solOptionItem.color)
              .css('border-color', solOptionItem.color)
        ...
        if ((this.config.multiple || this.config.allowNullSelection) && !$changedItem.prop('disabled')) {
          $quickDeleteItem = $('<span class="sol-quick-delete"/>')
              .html(this.config.texts.quickDelete)
              .click(function () {
                $changedItem.prop('checked', false)
                            .trigger('change');
              })
              .appendTo($existingDisplayItem);
          if (solOptionItem.color) {
            $quickDeleteItem.css('color', '#FFF');
          }
        }
...
* Copyright (c) 2019
*
* Custom method by Eventmaker
*
* Methods:
*   - setDefaultParams
*   - requiredToDataRequired
*       Because you can't submit a required element if not visible
*   - setSelectAllOption
*   - bindEvents
*   - isCheckboxesLimitReached:
*       if limitation is enabled, returns true if reached
*   - handleEdition
*   - selectedOptionOnEdit
*       Converting data on edit for SOL
*
* Created       : 2019-07-16 by @opheJansen
* Last modified : 2020-01-29 by @opheJansen
*/
function setDefaultParams($select) {
  $select.find('option:first').remove();
  var params = {
    maxHeight: '250px',
    useBracketParameters: true,
    texts: {
      searchplaceholder: $select.attr('data-placeholder'),
      noItemsAvailable: $select.attr('data-no-result')
    }
  };
  return params;
}

function requiredToDataRequired($select) {
  if ($select.attr('required') == "required") {
    $select.attr('data-required', true).removeAttr('required');
  }
}

function setSelectAllOption($select, params) {
  var selectUnselectAll = ($select.attr('data-selectall') == "true");
  params['showSelectAll'] = selectUnselectAll;
  params['allowNullSelectione'] = selectUnselectAll;
  if (selectUnselectAll == true) {
    params['texts']['selectAll'] = $select.attr('data-select-all-text');
    params['texts']['selectNone'] = $select.attr('data-select-none-text');
  }
  return params;
}

function bindEvents($select, params, afterInitCallback) {
  params['events'] = {
    onChange: function (sol, changedElements) {
      if (isCheckboxesLimitReached(sol)) {
        changedElements.prop("checked", false).change();
      } else {
        var selectedValues = $.map(sol.getSelection(), function(item){
          return $(item).val();
        }).join(";");
        $select.attr("data-value", selectedValues).change();
      }
    },
    onInitialized: function (sol) {
      if ($select.attr('multiple') == "multiple" && $(document).find('[data-show-if]').length > 0) {
        var elementName = $select.attr('name');

        if ($select.closest('[id*=linked-person-]').length > 0) {
          var linkedguestIndex = $select.closest("[id*=linked-person]").attr('id').split('linked-person-')[1];
          var person = "linked-person-" + linkedguestIndex;

          if (elementName.indexOf("guest_metadata") === -1) {
            var indexStartMetadataName = elementName.indexOf( '[' + linkedguestIndex + ']') + 3;
          } else {
            var indexStartMetadataName = elementName.indexOf( "[", elementName.indexOf('guest_metadata[', elementName.indexOf("[") + 1) + 1);
          }
          var indexEndMetadataName = elementName.indexOf(']', indexStartMetadataName);
        } else {
          var person = "main-person";

          if (elementName.indexOf("guest_metadata") === -1) {
            var indexStartMetadataName = elementName.indexOf('[');
          } else {
            var indexStartMetadataName = elementName.indexOf('[', elementName.indexOf("[") + 1);
          }
          var indexEndMetadataName = elementName.indexOf(']');
        }

        var metadataName = elementName.slice(indexStartMetadataName + 1, indexEndMetadataName);

        if (person != 'main-person') {
          var selectorShowIfs =  $('[id=' + person + '] [data-show-if*=' + metadataName + ']');
          var selectorRequiredIfs =  $('[id=' + person + '] [required_if*=' + metadataName + ']');
        } else {
          var selectorShowIfs =  $('[data-show-if*=' + metadataName + ']:not([id*=linked-person] [data-show-if*=' + metadataName + '])');
          var selectorRequiredIfs =  $('[required_if*=' + metadataName + ']:not([id*=linked-person] [required_if*=' + metadataName + '])');
        }

        if (typeof ShowIf === 'function') {
          selectorShowIfs.each(function() {
            new ShowIf($(this));
          });
        }

        selectorRequiredIfs.each(function() {
          new window.mobicheckin.registration.RequiredIf($(this));
        });
      }

      if (typeof afterInitCallback === 'function') {
        afterInitCallback.call(this);
      }
    }
  };
  return params;
...
function isCheckboxesLimitReached(sol) {
  var limit = sol.$originalElement.attr('data-limit');

  return limit && limit > -1 && sol.getSelection().length > limit;
}

function handleEdition($select, params) {
  if ($select.attr('data-value')) {
    var currentValues = $select.attr('data-value').split(';');
    params['converter'] = function (sol, rawData) {
      rawData.forEach(function (elem) {
        if (elem.type == "option") {
          selectedOptionOnEdit(currentValues, elem);
        } else if (elem.type == "optiongroup") {
          elem.children.forEach(function (child) {
            selectedOptionOnEdit(currentValues, child);
          });
        }
      });
      return rawData;
...
function selectedOptionOnEdit(currentValues, elem) {
  if (currentValues.includes(elem.value)) {
    elem.selected = true;
  }
}
