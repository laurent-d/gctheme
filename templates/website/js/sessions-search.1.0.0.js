class SessionsSearch {
  ...
  bindSearchEvents() {
    ...
    $(document).on("click", ".sessions-list a[data-thematic-id]", function() {
  ...
  singleChoiceForThematics(thematicId) {
    ...
    // .ghost classes only use into Grand-conference theme
    // Use for change style of thematics into the search part
    this.searchForm.find("label.ghost").removeClass("active"); // uncheck all thematics
    this.searchForm.find(".search-thematics [type=checkbox][value=" + thematicId + "]").closest("label.ghost").addClass("active");
  }
  ...
  extractSearchParams() {
    ...
    this.searchRecommendations = this.extractSimpleSearchParam(searchParams, "recommendations") == "true" || this.sessionsList.data("force-recommendations") === true;
    this.nbRecommendations = this.searchRecommendations && parseInt(this.sessionsList.data("nb-recommendations")) || 20;
    this.filterOnMissedSessions = this.sessionsList.data("filter-on-missed-sessions") === true || this.extractSimpleSearchParam(searchParams, "missed_sessions");
    this.rankAttribute = this.searchRecommendations ? "data-recommendation-distance" : "data-rank";
  }
  ...
  showItemIfMatchesSearch(i, $item) {
    ...
    const matchHasSession = !this.filterOnMissedSessions || ($item.data("has-streaming-archive") === true && $item.data("showed-up") !== true);
    ...
    if ((!this.searchRecommendations || i < this.nbRecommendations) && matchHasSession && matchTextSearch && matchDate && matchTimeslot && matchSessionTypes && matchThematics && matchLocations)
  ...
  sortItems() {
    ...
    this.sessionsList.each(function () {
      $(this).find(".session-item").sort(function (a, b) {
        return a.getAttribute(self.rankAttribute) - b.getAttribute(self.rankAttribute);
      }).appendTo($(this));
    });
  }
  ...
  search() {
    ...
    self.sessionsList.find(".session-item").sort(function (a, b) {
      return a.getAttribute(self.rankAttribute) - b.getAttribute(self.rankAttribute);
    }).each(function(i, item) {
      self.showItemIfMatchesSearch(i, $(item));
    });
